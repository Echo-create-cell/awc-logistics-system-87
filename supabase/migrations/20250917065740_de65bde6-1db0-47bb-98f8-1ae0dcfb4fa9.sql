-- Fix remaining critical security issues (Corrected)
...
BEGIN
  ...
  has_access := CASE 
    ...
    WHEN user_role_val IN ('admin', 'sales_director') THEN true
    -- Finance officers can view clients for invoicing
    WHEN user_role_val = 'finance_officer' THEN true
    -- Sales agents can only access clients they created or are assigned to
    WHEN user_role_val = 'sales_agent' AND 
