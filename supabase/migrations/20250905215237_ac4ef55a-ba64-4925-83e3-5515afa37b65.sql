...
WITH quotation_data AS (
  ...
  AND id NOT IN (SELECT DISTINCT quotation_id FROM quotation_commodities WHERE quotation_id IS NOT NULL)
)
...
WITH won_quotations AS (
  ...
  WHERE status = 'won' 
  AND client_quote > 0
  AND id NOT IN (SELECT DISTINCT quotation_id FROM invoices WHERE quotation_id IS NOT NULL)
)
...
WITH invoice_data AS (
  ...
  AND i.id NOT IN (SELECT DISTINCT invoice_id FROM invoice_items WHERE invoice_id IS NOT NULL)
)
